...
void MagixGUI::initialize(SceneManager *sceneMgr, RenderWindow *window, MagixExternalDefinitions *def, MagixGameStateManager *gameStateMgr, MagixItemManager *itemManager, MagixCritterManager *critterMgr, MagixUnitManager *unitManager, MagixNetworkManager *networkManager, MagixCamera *camera, MagixSoundManager *soundMgr, MagixEffectsManager *effectsMgr, MagixCollisionManager *collisionMgr)
{
	...
	mButton[BUTTON_TEXTINPUT3] = OverlayManager::getSingleton().getOverlayElement("GUI/TextInputBoxButton4");
	mButtonText[BUTTON_TEXTINPUT3] = OverlayManager::getSingleton().getOverlayElement("GUI/TextInputBoxButtonText4");
	mButton[BUTTON_TEXTINPUT3] = OverlayManager::getSingleton().getOverlayElement("GUI/TextInputBoxButton5");
	mButtonText[BUTTON_TEXTINPUT3] = OverlayManager::getSingleton().getOverlayElement("GUI/TextInputBoxButtonText5");
...
void MagixGUI::scrollMouse(int x, int y, int winWidth, int winHeight, bool setPosition)
{
	if (setPosition)mCursor->setPosition((Real)x, (Real)y);
...
void MagixGUI::update(const FrameEvent &evt)
{
	...
	// ДОБАВЬТЕ ЭТОТ БЛОК - проверка нажатия E
	static bool eKeyWasUp = true;

	// Используем hotkeyHeld для регистрации нажатия
	mInputManager->hotkeyHeld(OIS::KC_E);

	if (mInputManager->getHeldHotkey() == OIS::KC_E)
	{
		if (eKeyWasUp)
		{
			pickUpNearestItem();
...
void MagixGUI::updateCinematicBars(const FrameEvent &evt)
{
	...
	if (cinematicBarsTimeout>0)
	{
		...
		mBackdrop->setDimensions(1.5, (Real)(1 + 0.5*(cinematicBarsTimeout / 2)));
	...
	else if (cinematicBarsTimeout<0)
	{
		...
		{
			mBackdrop->setDimensions(1.5, (Real)(1 + 0.5*((2 + cinematicBarsTimeout) / 2)));
			mBackdrop->setPosition(-0.25, (Real)(-0.25*((2 + cinematicBarsTimeout) / 2)));
		}
	}
}
...
void MagixGUI::updateLogo(const FrameEvent &evt)
{
	...
	if (mGameStateManager->getGameState() == GAMESTATE_STARTSCREEN && logoCount <= 5)
	{
		if (logoCount == 0)
		{
			...
			OverlayManager::getSingleton().getOverlayElement("GUI/UpdateBox")->hide();
			OverlayManager::getSingleton().getOverlayElement("GUI/StartScreenTitle")->setDimensions(8, 8);
			OverlayManager::getSingleton().getOverlayElement("GUI/StartScreenTitle")->setPosition(-3.5, -0.2);
			mBackdrop->setMaterialName("GUIMat/Backdrop1");
		}
		logoCount += evt.timeSinceLastFrame;
		...
		if (logoCount <= 5)
		{
			...
			OverlayManager::getSingleton().getOverlayElement("GUI/StartScreenTitle")->setDimensions((Real)(0.8 + 0.8 * 9 * tRatio), (Real)(0.12 + 0.12 * 9 * tRatio));
			OverlayManager::getSingleton().getOverlayElement("GUI/StartScreenTitle")->setPosition((Real)(0.5 - (0.8 + 0.8 * 9 * tRatio) / 2), (Real)(0.25 - (0.12 + 0.12 * 9 * tRatio) / 2));
		}
		if (logoCount >= 5)
		{
...
void MagixGUI::updateSelectPanel(const FrameEvent &evt)
{
	if (selectPanelTimeout>0)
	{
		...
		mSelectPanel->setPosition((Real)(mSelectPanel->getLeft() + mSelectPanel->getWidth()*0.5), (Real)(mSelectPanel->getTop() + mSelectPanel->getHeight()*0.5));
...
void MagixGUI::mouseLeftDown()
{
	...
	if (mHoverWindow)
	{
		...
		if (mHoverWindow == mBox[GUI_FRIENDBOX])	//friendbox options
		{
			...
			mBox[GUI_LISTSELECTBOX]->setPosition((Real)(cursorX + 0.02), (Real)(cursorY + 0.02));
			mListSelectHighlight->setDimensions(mBox[GUI_FRIENDBOX]->getWidth(), 0.02);
			mListSelectHighlight->setPosition(mBox[GUI_FRIENDBOX]->getLeft(), (Real)(mBox[GUI_FRIENDBOX]->getTop() + tFontHeight*tApparentLine + 0.01));
		...
		if (mHoverWindow == mBox[GUI_EMOTEBOX])	//emotebox options
		{
			...
			mListSelectHighlight->setPosition(mBox[GUI_EMOTEBOX]->getLeft(), (Real)(mBox[GUI_EMOTEBOX]->getTop() + tFontHeight*tApparentLine + 0.01));
			mListSelectHighlight->show();
			showListSelectBox(true);
			mBox[GUI_LISTSELECTBOX]->hide();
...
#pragma warning(pop)
		...
		if (mHoverWindow == mBox[GUI_ITEMBOX])	//itembox options
		{
			...
			mListSelectHighlight->setPosition(mBox[GUI_ITEMBOX]->getLeft(), (Real)(mBox[GUI_ITEMBOX]->getTop() + tFontHeight*tApparentLine + 0.01));
			mListSelectHighlight->show();
			showListSelectBox(true);
			...
			if (listButtonData != "")
			{
				...
				mBox[GUI_LISTSELECTBOX]->setDimensions(0.1, (Real)(0.04*(mButton[BUTTON_LISTSELECT3]->isVisible() ? 3 : 2)));
				mBox[GUI_LISTSELECTBOX]->setPosition((Real)(cursorX + 0.02), (Real)(cursorY + 0.02));
			}
		...
		if (mHoverWindow == mBox[GUI_STASHBOX])	//stashbox options
		{
			...
			mListSelectHighlight->setPosition(mBox[GUI_STASHBOX]->getLeft(), (Real)(mBox[GUI_STASHBOX]->getTop() + tFontHeight*tApparentLine + 0.01));
			mListSelectHighlight->show();
			showListSelectBox(true);
...
#include <sstream>
				...
				else if (tParam == COMMAND_SPAWN && mDef->isAdmin)
				{
					...
					if (tC.type != "")
					{
						...
						int numCritters = 1; // Default number of critters to spawn
						size_t colonPos = tCaption.find(':');
						if (colonPos != std::string::npos)
						{
							std::string numCrittersStr = tCaption.substr(colonPos + 1);
							try
							{
								numCritters = std::stoi(numCrittersStr);
							}
						...
						{
							if (mGameStateManager->isCampaign())
								mUnitManager->createCritter(mCritterManager->getNextEmptyID(0), tC.type, Vector3(tX, tY, tZ));
							else
								mNetworkManager->sendCritterCreate(tC.type, Vector3(tX, tY, tZ), false);
						}
					}
				}
...
void MagixGUI::sizeWindowDrag(unsigned short sizeDir, const Real &minWidth, const Real &minHeight)
{
...
void MagixGUI::resizeScrollbarByParent(OverlayElement *parent, const Real &dwidth, const Real &dheight)
{
	for (int i = 0; i<MAX_SCROLLERS; i++)
	{
		if (mScroller[i])
...
void MagixGUI::updateBoxScrollerCaption(const unsigned short &boxID, const unsigned short &scrollerID, const vector<String>::type &captionList)
{
...
const short MagixGUI::getListButtonLine(const unsigned short &boxID, const unsigned short &scrollerID, const unsigned short &listLines, Real &fontHeight, short &apparentLine)
{
...
const short MagixGUI::getListButtonLine(const unsigned short &boxID, Real &fontHeight, short &apparentLine)
{
	if (boxID >= MAX_BOXES)return -1;
	if (!mBox[boxID] || !mBoxText[boxID])return -1;
...
void MagixGUI::showCreditsCaption(const String &caption)
{
	...
	tCaption->setTop((Real)(0.5 - StringConverter::parseReal(tCaption->getParameter("char_height"))*int(tLines.size()) / 2));
}
...
bool MagixGUI::checkItemEquippable(const String &meshName, bool isCreate, bool doEquip, bool ground)
{
...
void MagixGUI::pickUpNearestItem()
{
	if (!mGameStateManager->isInGame() || mGameStateManager->isPaused() || mGameStateManager->isCinematic())
		return;

	const Vector3 playerPos = mUnitManager->getPlayer()->getPosition();

	MagixItem* nearestItem = nullptr;
	Real nearestDistance = 50.0f;

	const vector<MagixItem*>::type items = mItemManager->getItemList();

	for (MagixItem* item : items)
	{
		if (!item || !item->getObjectNode())
			continue;

		Real distance = playerPos.distance(item->getPosition());
	...
	// Если нашли подходящий предмет - заставляем персонажа бежать к нему
	if (nearestItem)
	{
		// Устанавливаем предмет как цель для автоподбора
		mUnitManager->getPlayer()->setAutoTrackObject(nearestItem);

		// Устанавливаем позицию предмета как цель для бега
		mUnitManager->getPlayer()->setTarget(nearestItem->getPosition(), TARGET_RUNTOPICKUP);

		// Отменяем автоатаку если она была
		mUnitManager->getPlayer()->setAutoAttack(0);
