...
void MagixUnit::setColours(const ColourValue &pelt, const ColourValue &underfur, const ColourValue &mane, const ColourValue &nose, const ColourValue &eyeTop, const ColourValue &eyeBot, const ColourValue &ears, const ColourValue &tailtip, const ColourValue &eyes, const ColourValue &wing, const ColourValue &marking)
{
...
void MagixUnit::updateAnimation(const FrameEvent &evt, MagixExternalDefinitions *def)
{
	...
	else if (blinkFlag != 0)
	{
		...
		if (mHeadEnt->getAnimationState("Blink")->getTimePosition() >= eyelidState*mHeadEnt->getAnimationState("Blink")->getLength())
			mHeadEnt->getAnimationState("Blink")->addTime((Real)(blinkFlag*evt.timeSinceLastFrame*1.5));
	...
	if (freeLook)
	{
		...
		if (headTurnCount>0)
		{
			...
			else
			{
				...
				if (mManeEnt)mManeEnt->getAnimationState(tOppTurn)->setTimePosition((Real)(headTurnCount*(mManeEnt->getAnimationState(tOppTurn)->getLength()*0.99)));
				if (mTuftEnt)mTuftEnt->getAnimationState(tOppTurn)->setTimePosition((Real)(headTurnCount*(mTuftEnt->getAnimationState(tOppTurn)->getLength()*0.99)));
			}
		}
		mBodyEnt->getAnimationState(tTurnAnim)->setEnabled(true);
		mBodyEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)((1 - headTurnCount)*tTurnRatio*(mBodyEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		//if(mHeadEnt->getAnimationState(tOppTurn)->getEnabled())mHeadEnt->getAnimationState(tOppTurn)->setEnabled(false);
		mHeadEnt->getAnimationState(tTurnAnim)->setEnabled(true);
		mHeadEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)((1 - headTurnCount)*tTurnRatio*(mHeadEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		if (mManeEnt)
		{
			//if(mManeEnt->getAnimationState(tOppTurn)->getEnabled())mManeEnt->getAnimationState(tOppTurn)->setEnabled(false);
			mManeEnt->getAnimationState(tTurnAnim)->setEnabled(true);
			mManeEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)((1 - headTurnCount)*tTurnRatio*(mManeEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		}
		if (mTuftEnt)
		{
			//if(mManeEnt->getAnimationState(tOppTurn)->getEnabled())mManeEnt->getAnimationState(tOppTurn)->setEnabled(false);
			mTuftEnt->getAnimationState(tTurnAnim)->setEnabled(true);
			mTuftEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)((1 - headTurnCount)*tTurnRatio*(mTuftEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		}
		...
		if (tTurnEyes>0)
		{
			...
			if (mHeadEnt->getAnimationState(tOppTurn)->getEnabled())mHeadEnt->getAnimationState(tOppTurn)->setEnabled(false);
			mHeadEnt->getAnimationState(tTurnAnim)->setEnabled(true);
			mHeadEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)(tTurnEyes*(mHeadEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		}
		else
		{
		...
		mBodyEnt->getAnimationState(tTurnAnim)->setEnabled(true);
		mBodyEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)(tTurnRatio*(mBodyEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		if (mHeadEnt->getAnimationState(tOppTurn)->getEnabled())mHeadEnt->getAnimationState(tOppTurn)->setEnabled(false);
		mHeadEnt->getAnimationState(tTurnAnim)->setEnabled(true);
		mHeadEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)(tTurnRatio*(mHeadEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		if (mManeEnt)
		{
			if (mManeEnt->getAnimationState(tOppTurn)->getEnabled())mManeEnt->getAnimationState(tOppTurn)->setEnabled(false);
			mManeEnt->getAnimationState(tTurnAnim)->setEnabled(true);
			mManeEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)(tTurnRatio*(mManeEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		}
		if (mTuftEnt)
		{
			if (mTuftEnt->getAnimationState(tOppTurn)->getEnabled())mTuftEnt->getAnimationState(tOppTurn)->setEnabled(false);
			mTuftEnt->getAnimationState(tTurnAnim)->setEnabled(true);
			mTuftEnt->getAnimationState(tTurnAnim)->setTimePosition((Real)(tTurnRatio*(mTuftEnt->getAnimationState(tTurnAnim)->getLength()*0.99)));
		}
	...
	if (animCount>0)
	{
		...
		else
		{
			if (prevAnimBase != "")
			{
				...
				if (mManeEnt)mManeEnt->getAnimationState(prevAnimBase)->setWeight((Real)(animCount / 0.1));
				mTailEnt->getAnimationState(prevAnimBase)->setWeight((Real)(animCount / 0.1));
				if (mWingEnt)mWingEnt->getAnimationState(prevAnimBase)->setWeight((Real)(animCount / 0.1));
				if (mTuftEnt)mTuftEnt->getAnimationState(prevAnimBase)->setWeight((Real)(animCount / 0.1));
			}
			if (animBase != "")
			{
				...
				if (mManeEnt)mManeEnt->getAnimationState(animBase)->setWeight((Real)((0.1 - animCount) / 0.1));
				mTailEnt->getAnimationState(animBase)->setWeight((Real)((0.1 - animCount) / 0.1));
				if (mWingEnt)mWingEnt->getAnimationState(animBase)->setWeight((Real)((0.1 - animCount) / 0.1));
				if (mTuftEnt)mTuftEnt->getAnimationState(animBase)->setWeight((Real)((0.1 - animCount) / 0.1));
			}
		}
	}
}
	...
	if (animSecondCount>0)
	{
		...
		else
		{
			if (prevAnimSecond != "")
			{
				...
				mBodyEnt->getAnimationState(prevAnimSecond)->setWeight((Real)(animSecondCount / 0.1));
				mHeadEnt->getAnimationState(prevAnimSecond)->setWeight((Real)(animSecondCount / 0.1));
				if (mManeEnt)mManeEnt->getAnimationState(prevAnimSecond)->setWeight((Real)(animSecondCount / 0.1));
				mTailEnt->getAnimationState(prevAnimSecond)->setWeight((Real)(animSecondCount / 0.1));
				if (mWingEnt)mWingEnt->getAnimationState(prevAnimSecond)->setWeight((Real)(animSecondCount / 0.1));
				if (mTuftEnt)mTuftEnt->getAnimationState(prevAnimSecond)->setWeight((Real)(animSecondCount / 0.1));
			}
			if (animSecond != "")
			{
				mBodyEnt->getAnimationState(animSecond)->setWeight((Real)((0.1 - animSecondCount) / 0.1));
				mHeadEnt->getAnimationState(animSecond)->setWeight((Real)((0.1 - animSecondCount) / 0.1));
				if (mManeEnt)mManeEnt->getAnimationState(animSecond)->setWeight((Real)((0.1 - animSecondCount) / 0.1));
				mTailEnt->getAnimationState(animSecond)->setWeight((Real)((0.1 - animSecondCount) / 0.1));
				if (mWingEnt)mWingEnt->getAnimationState(animSecond)->setWeight((Real)((0.1 - animSecondCount) / 0.1));
				if (mTuftEnt)mTuftEnt->getAnimationState(animSecond)->setWeight((Real)((0.1 - animSecondCount) / 0.1));
			}
		}
	}
}
void MagixUnit::updateMovement(const FrameEvent &evt)
{
	...
	if (jump && distanceFromGround()<(MAX_JUMPHEIGHT*hasDoubleJumped ? 2 : 1) && mForce.y >= 0)
		addForce(Vector3(0, /*JUMP_GRAVITY*evt.timeSinceLastFrame*/ + (distanceFromGround() <= GROUND_THRESHOLD ? JUMP_ACCEL : 0), 0));
	//Doublejump
...
void MagixUnit::updateAutoTrack()
{
	if (mAutoTrackTarget)
	{
		...
		if (!isAttacking() && (isPositionChanged() || mAutoTrackTarget->getPosition() != lookTargetVector))
		{
			...
			{
				const Real tHeight = (Real)(static_cast<MagixLiving*>(mAutoTrackTarget)->getHeight()*0.5);
...
void MagixUnit::setTarget(const Real &x, const Real &y, const Real &z, const unsigned char &action)
{
	...
	if (action == TARGET_RUNTO || action == TARGET_LOOKAT || action == TARGET_RUNTOPICKUP)
	{
		...
		Radian tYaw = Degree(0);
		if (tZ != 0)tYaw = Math::ATan(tX / tZ);
		else tYaw = Degree(tX >= 0 ? (Real)90 : (Real)270);
		if (tX<0)tYaw += Degree(360);
		if (tZ<0)tYaw += Degree(180);
		setYaw(Degree(tYaw));
	}
...
void MagixUnit::changeAnimBase(const String &type, const Real &speed, bool resetTimePosition, bool reverseAnimation, bool loop)
{
...
void MagixUnit::changeAnimSecond(const String &type, const Real &speed, bool resetTimePosition, bool reverseAnimation, bool loop)
{
...
void MagixUnit::updateEmote(const FrameEvent &evt, MagixExternalDefinitions *def)
{
	if (emoteCount>0)
	{
		...
		setEmoteState(emoteMode, (Real)((EMOTE_TRANSITIONTIME - emoteCount) / EMOTE_TRANSITIONTIME), def);
	}
}
...
void MagixUnit::setEmoteState(const String &mode, const Real &weight, MagixExternalDefinitions *def)
{
...
void MagixUnit::replaceManeMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial)
{
...
void MagixUnit::replaceTailMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial)
{
	if (!mTailEnt)return;
...
void MagixUnit::replaceWingMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial)
{
...
void MagixUnit::replaceTuftMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial)
{
...
void MagixUnit::setCustomAnimation(bool flag, const String &animName, Real animSpeed, bool loop, bool reverse)
{
...
void MagixUnit::equip(SceneManager *sceneMgr, MagixExternalDefinitions *mDef, const String &meshName, const Vector3 &offset, const short &slot, bool altAnim)
{
	if (unitID == 0)return;
...
const String MagixUnit::unequip(SceneManager *sceneMgr, MagixExternalDefinitions *mDef, const unsigned short &iID)
{
...
void MagixUnit::setMarkings(const unsigned char &bodyID, const unsigned char &headID, const unsigned char &tailID)
{
	if (matGroupName != "CustomMat")return;
	bodyMarkID = bodyID;
	headMarkID = headID;
