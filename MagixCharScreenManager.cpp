...
void MagixCharScreenManager::initializeText(const unsigned short &i, OverlayElement *text)
{
	mText[i] = text;
}
void MagixCharScreenManager::initializeBackNextButtons(OverlayElement *back, OverlayElement *next, OverlayElement *del, OverlayElement *backText, OverlayElement *nextText, OverlayElement *delText)
{
	mBackButton = back;
	mNextButton = next;
	mDeleteButton = del;
...
void MagixCharScreenManager::initializeLRButtons(const unsigned short &i, OverlayElement *lButton, OverlayElement *rButton)
{
	mLButton[i] = lButton;
	mRButton[i] = rButton;
}
void MagixCharScreenManager::initializeSliders(const unsigned short &i, OverlayElement *slider, OverlayElement *sliderMarker)
{
	mSlider[i] = slider;
...
void MagixCharScreenManager::reset(bool load)
{
	...
	tailMarkID = 0;
	colourPartID = 0;
	colourVal[0] = ColourValue(0.45, 0.45, 0.45 ); //Pelt
	colourVal[1] = ColourValue(0.80, 0.80, 0.80); //Underfur
	colourVal[2] = ColourValue(0.20, 0.20, 0.20); //Mane
	colourVal[3] = ColourValue(1, 0.70, 0.70); //Nose
	colourVal[4] = ColourValue(0.45, 0.45, 0.45); //Above eyes
	colourVal[5] = ColourValue(0.80, 0.80, 0.80); //Below eyes
	colourVal[6] = ColourValue(1, 0.70, 0.70); //Ears
	colourVal[7] = ColourValue(0.20, 0.20, 0.20); //Tailtip
	colourVal[8] = ColourValue(0.90, 0.20, 0.20); //Eyes
	...
	if (button == mDeleteButton)
	{
		...
		if (page == PAGE_EDIT)
		{
			if (deleteConfirmation == "")
			{
				...
				mName->show();
				mAlertBox->showAlert("Enter character name to confirm delete", (Real)(mName->getLeft() + 0.25), (Real)(mName->getTop() + mName->getHeight() + 0.05));
				return 0;
			}
			if (deleteConfirmation != getName())
			{
				mAlertBox->showAlert("Character name mismatch", (Real)(mName->getLeft() + 0.25), (Real)(mName->getTop() + mName->getHeight() + 0.05));
				return 0;
			}
...
void MagixCharScreenManager::changeTrait(const unsigned short &iID, bool increment)
{
	...
	else if (iID == 5)
	{
		...
		mUnit->setMarkings((unsigned char)bodyMarkID, (unsigned char)headMarkID, (unsigned char)tailMarkID);
	}
	//head mark
	else if (iID == 6)
	{
...
void MagixCharScreenManager::applyRestrictions(const String &partChanged, bool increment)
{
	if (partChanged == "Head")
	{
		//back to maneless
...
const unsigned short MagixCharScreenManager::nextPage()
{
	...
	if ((page == PAGE_CUSTOM1 || page == PAGE_PRESET1) && (!validateName() || getName().length() == 0 || getName()[0] == ' '))
	{
		...
		mAlertBox->showAlert(tAlert, (Real)(mName->getLeft() + 0.25), (Real)(mName->getTop() + mName->getHeight() + 0.05));
		return 0;
	}
...
void MagixCharScreenManager::updateSliders(const Real &one, const Real &two, const Real &three)
{
	if (page == PAGE_CUSTOM3)
	{
	...
	{
		const Real tScale = (Real)(0.25 + (page == PAGE_PRESET2 ? two : one)*1.25);
...
bool MagixCharScreenManager::update(const FrameEvent &evt, const Real &x, const Real &y)
{
	...
	if (page == PAGE_SELECT)
	{
		...
		if (startGameCount>0)
		{
			...
			const Vector3 tVect = mUnit->getPosition(true) + mUnit->getObjectNode()->getOrientation()*Vector3(0, 1, 4) - Vector3(2090.37, 195.819, 2454.9);
			mSceneMgr->getCamera("PlayerCam")->setPosition(Vector3(2090.37, 195.819, 2454.9) + tVect*(3 - startGameCount) / 3 * 0.25);
			if (startGameCount <= 0)
			{
				startGameCount = 0;
	...
	if (isRotateUnit)
	{
		...
		rotateStartX = x;
		rotateStartY = y;
		if (Math::Abs(dYaw)>Degree(15))dYaw = Degree((Real)(15 * dYaw<Degree(0) ? -1 : 1));
	...
	if (mUnit->getObjectNode())
	{
		...
		//Camera zoom
		const Vector3 tVect = mUnit->getPosition(true) + mUnit->getObjectNode()->getOrientation()*Vector3(0, 1, 4) - Vector3(2075, 200, 2460);
		mSceneMgr->getCamera("PlayerCam")->setPosition(Vector3(2075, 200, 2460) + tVect*camZoom);
	}
	return false;
}
...
{
	mSceneMgr->getCamera("PlayerCam")->setOrientation(Quaternion(Degree(-350), Vector3::UNIT_Y));
	if (!mUnit->getObjectNode())return;
	mUnit->setPosition(2074, 185, 2379);
	mUnit->resetTarget();
	mUnit->setGroundHeight(185);
	mUnit->yaw(Degree(10));
	mUnit->getObjectNode()->yaw(Degree(10));
	mUnit->setFreeLook(false, false);
}
void MagixCharScreenManager::positionUnitForSelect()
{
	mSceneMgr->getCamera("PlayerCam")->setOrientation(Quaternion(Degree(-350), Vector3::UNIT_Y));
	mSceneMgr->getCamera("PlayerCam")->setPosition(2090.37, 195.819, 2454.9);
	if (!mUnit->getObjectNode())return;
	const Real tDist = (mUnit->getObjectNode()->getScale().x - 1)*-11;
	mUnit->setPosition((Real)(2068 - (tDist<0 ? tDist : 0)*0.6), (Real)(180 + (tDist<0 ? tDist : 0)), (Real)(2376 + (tDist<0 ? tDist : 0)*0.6));
	mUnit->resetTarget();
	mUnit->setGroundHeight(189 + (tDist<0 ? tDist : 0));
	mUnit->yaw(Degree(-30));
	mUnit->getObjectNode()->yaw(Degree(-30));
	mUnit->setFreeLook(false, false);
