...
ogremagixFrameListener::ogremagixFrameListener(MagixHandler *magixHandler, SceneManager *sceneMgr, RenderWindow* win, Camera* cam)
...
bool ogremagixFrameListener::mouseMoved(const OIS::MouseEvent &e)
{
	...
	if (!mMagixHandler->getGameState()->isCinematic() && mMagixHandler->getGameState()->isInGame() && mMagixHandler->getCameraMode() == CAMERA_LOCKED)
	{
		...
		if (ms.buttonDown(MB_Right) && !mMagixHandler->getCamera()->getIsFirstPerson())
		{
			...
			{
				mMagixHandler->getCamera()->doCameraZoom((Real)((Math::Abs((Real)ms.X.rel)>8000 ? 0 : -ms.Y.rel) * 0.0013 * mouseSense));
			}
			else
			{
		...
		else if (!mMagixHandler->getPlayer()->getIsLockedOn())
		{
			if (mMagixHandler->getMagixGUI()->isTablet())
			{
				mMagixHandler->getCamera()->yaw(Degree((Real)((Math::Abs((Real)ms.X.rel)>8000 ? 0 : -ms.X.rel) * 0.0013 * mouseSense)));
				mMagixHandler->getCamera()->pitch(Degree((Real)((Math::Abs((Real)ms.Y.rel)>8000 ? 0 : -ms.Y.rel) * 0.0013 * mouseSense)));
			}
			else
			{
	...
	else//Mouse cursor mode
	{
		...
		else
		{
			...
			else
			{
				...
				{
					mMagixHandler->getMagixGUI()->scrollMouse((int)(ms.X.rel * 1 * mouseSense), (int)(ms.Y.rel * 1 * mouseSense), mWindow->getWidth(), mWindow->getHeight());
				}
			}
		}
	}
	...
	if (mMagixHandler->getMagixGUI()->isTablet() && !mMagixHandler->getMagixGUI()->getUseWindowsCursor())
	{
		...
		mMagixHandler->getMagixGUI()->scrollMouse((int)(ms.X.rel * 0.015 * tXYRatio * mouseSense), (int)(ms.Y.rel * 0.015 * mouseSense), mWindow->getWidth(), mWindow->getHeight());
	}
...
bool ogremagixFrameListener::keyPressed(const OIS::KeyEvent &e)
{
		...
		switch (e.key)
	{
		...
		case KC_E:
			...
			{
				// ДОБАВЬТЕ ЭТО - подбор предмета по клавише E
				mMagixHandler->getMagixGUI()->pickUpNearestItem();
			}
			break;
		case KC_1:
