...
#include <string>
	...
	if (StringUtil::startsWith(caption, COMMAND_SPAWN))
	{
		...
		// Extract the critter type and number of critters to spawn
		std::string critterType;
		int numCritters = 1; // Default number of critters to spawn

		// Check if the caption contains a colon character
		size_t colonPos = caption.find(':');
		if (colonPos != std::string::npos)
		{
			critterType = caption.substr(0, colonPos);
			std::string numCrittersStr = caption.substr(colonPos + 1);
		...
		{
			// Spawn logic for a single critter
			// Use the critterType variable to determine which critter to spawn
			// ...
		}
