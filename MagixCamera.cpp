...
void MagixCamera::initialize(SceneManager *sceneMgr, MagixPlayer *player, MagixCollisionManager *collMgr)
{
	...
	// ÏÎËÓ×ÀÅÌ ÈËÈ ÑÎÇÄÀÅÌ ÎÑÍÎÂÍÓÞ ÊÀÌÅÐÓ
	if (!sceneMgr->hasCamera("PlayerCam"))
	{
	...
	// ÑÎÇÄÀÅÌ ÊÀÌÅÐÓ ÎÒÐÀÆÅÍÈÉ (åñëè åùå íå ñîçäàíà)
	if (!sceneMgr->hasCamera("ReflectCamera"))
	{
		mReflectCamera = sceneMgr->createCamera("ReflectCamera");
		mReflectCamera->setNearClipDistance(mCamera->getNearClipDistance());
		mReflectCamera->setFarClipDistance(mCamera->getFarClipDistance());
		mReflectCamera->setAspectRatio(mCamera->getAspectRatio());
	}
	else
	{
		mReflectCamera = sceneMgr->getCamera("ReflectCamera");
	}
	...
	mCamNode->attachObject(mReflectCamera);
}

void MagixCamera::ReplicateMainCamera()
{
	mReflectCamera->setOrientation(mCamera->getOrientation());
	mReflectCamera->setPosition(mCamera->getPosition());

	// Êîïèðóåì âñå ïàðàìåòðû êàìåðû
	mReflectCamera->setNearClipDistance(mCamera->getNearClipDistance());
	mReflectCamera->setFarClipDistance(mCamera->getFarClipDistance());
	mReflectCamera->setFOVy(mCamera->getFOVy());
	mReflectCamera->setAspectRatio(mCamera->getAspectRatio());
}
...
void MagixCamera::update(const FrameEvent &evt)
{
	...
	if (camMode == CAMERA_LOCKED || camMode == CAMERA_FREECURSOR || camMode == CAMERA_FREECURSORNEW)
	{
		...
		{
			Quaternion tCamOrientationBuffer = getCameraOrientation();
	...
	if (turnUp)pitch(Degree((Real)(45 * camSpeed*0.02*evt.timeSinceLastFrame)));
	else if (turnDown)pitch(Degree((Real)(-45 * camSpeed*0.02*evt.timeSinceLastFrame)));
	if (turnLeft)yaw(Degree((Real)(45 * camSpeed*0.02*evt.timeSinceLastFrame)));
	else if (turnRight)yaw(Degree((Real)(-45 * camSpeed*0.02*evt.timeSinceLastFrame)));
...
void MagixCamera::setCameraMode(const unsigned char &mode, bool firstPerson, bool forceUpdate)
{
	...
	case CAMERA_LOCKED:
		...
		if (!mReflectCamera->isAttached())
		{
			mCamNode->attachObject(mReflectCamera);
			mReflectCamera->setPosition(0, 0, 0);
		}
		if (!isFirstPerson)
		{
			doCameraZoom(0);
	...
	case CAMERA_FREE:
		...
		if (mReflectCamera->isAttached())mCamNode->detachObject(mReflectCamera);
...
void MagixCamera::addCameraTrackKeyFrame(const Real &timePosition, const Vector3 &translation, const Quaternion &rotation)
{
...
void MagixCamera::doCameraTrack(bool loop, bool autoTrack, SceneNode *node)
{
	...
	if (camMode != CAMERA_FREE && mReflectCamera->isAttached())mCamNode->detachObject(mReflectCamera);
	mCamTrackNode->attachObject(mReflectCamera);
	if (autoTrack)
	{
...
void MagixCamera::stopCameraTrack()
{
	...
	if (mReflectCamera->isAttached() && mReflectCamera->getParentSceneNode() == mCamTrackNode)
	{
		mCamTrackNode->detachObject(mReflectCamera);
		if (camMode != CAMERA_FREE)mCamNode->attachObject(mReflectCamera);
	}
}
