...
void MagixExternalDefinitions::loadSettings(bool &isTablet, Real &mouseSensitivity, Real &musicVol, Real &guiVol, Real &soundVol, bool &isShadowsOn, bool &isBloom, String &username, bool &generalCharname, bool &localUsername, bool &channelBlink, bool &runMode, bool &doubleJumpOn, bool &autoLipSync, bool &useWindowsCursor)
{
	unsigned int tSize = 0;
	char *tBuffer;
	String tData = "";
...
void MagixExternalDefinitions::saveSettings(bool isTablet, const Real &mouseSensitivity, const Real &musicVol, const Real &guiVol, const Real &soundVol, const bool &isShadowsOn, bool isBloom, const String &username, bool generalCharname, bool localUsername, bool channelBlink, bool runMode, bool doubleJumpOn, bool autoLipSync, bool useWindowsCursor)
{
...
bool MagixExternalDefinitions::loadWorld(const String &filename, String &terrain, Real &x, Real &z, Vector3 &spawnSquare, Vector2 &worldBounds)
{
	unsigned int tSize = 0;
	char *tBuffer;
	String tData = "";
...
bool MagixExternalDefinitions::loadCampaign(const String &name, CampaignEventList &data, String &fileName, bool customCampaigns)
{
	unsigned int tSize = 0;
	char *tBuffer;
	String tData = "";
...
bool MagixExternalDefinitions::loadHomePoint(String &mapName, Vector2 &point, unsigned char &dimension, bool isSavePoint)
{
	std::ifstream inFile;
...
void MagixExternalDefinitions::saveSavePoint(const String &mapName, const Vector2 &point, const unsigned char &dimension)
{
	saveHomePoint(mapName, point, dimension, true);
}
void MagixExternalDefinitions::saveHomePoint(const String &mapName, const Vector2 &point, const unsigned char &dimension, bool isSavePoint)
{
	std::ofstream outFile;
...
void MagixExternalDefinitions::loadWeather(const String &type, String &particle, Vector3 &offset, String &sound, bool isCustom)
{
	unsigned int tSize = 0;
	char *tBuffer;
	String tData = "";
...
const unsigned char MagixExternalDefinitions::getCritterRandomAttack(const String &type, bool *hitAlly)
{
	for (int i = 0; i<(int)critterList.size(); i++)
	{
		if (critterList[i].type == type)
		{
			...
			unsigned char tID = (unsigned char)Math::RangeRandom(1, (Real)critterList[i].attackList.size());
...
const unsigned char MagixExternalDefinitions::getCritterRandomSpecificAttack(const String &type, bool getNonHeal)
{
	for (int i = 0; i<(int)critterList.size(); i++)
	{
		if (critterList[i].type == type)
		{
			...
			unsigned char tID = (unsigned char)Math::RangeRandom(1, (Real)tList.size());
...
const std::pair<CritterAttack, String> MagixExternalDefinitions::getCritterAttackAndSound(const String &type, const unsigned char &iID)
{
	for (int i = 0; i<(int)critterList.size(); i++)
	{
		if (critterList[i].type == type)
