	...
	MagixCamera *mainCam)
{
	...
	if (!mSceneMgr->hasCamera("ReflectCamera"))
	{
		...
		mReflectCamera->setNearClipDistance(1.0f);
		mReflectCamera->setFarClipDistance(20000.0f);
		mReflectCamera->setAspectRatio(1.3333f);
		mReflectCamera->setFOVy(Degree(60));
	}
	else
	{
...
void MagixWorld::initReflectionViewport()
{
	removeReflectionViewport();

	if (mReflectRtt)
	{
		Viewport *v = mReflectRtt->addViewport(mReflectCamera);
		...
		{
			if (mUnderwaterTexState)
				mUnderwaterTexState->setProjectiveTexturing(true, mReflectCamera);
		}
		else
		{
			if (mReflectTexState)
				mReflectTexState->setProjectiveTexturing(true, mReflectCamera);
		}
	}
}

void MagixWorld::removeReflectionViewport()
{
	if (mReflectTexState)
...
void MagixWorld::updateReflectionCamera()
{
	...
	// ИСПРАВЛЕНИЕ: Более точное отражение позиции относительно плоскости воды
	float waterY = RefPosition.y;
	Vector3 reflectPos = mainCamPos;

	// Рассчитываем расстояние от камеры до водной поверхности
	float distanceToWater = mainCamPos.y - waterY;

	// Отражаем позицию: если камера выше воды, отражение должно быть ниже воды на том же расстоянии
	if (mMainCamera->getIsUnderwater())
	{
		// Если под водой - отражение над водой
		reflectPos.y = waterY + abs(distanceToWater);
	}
	else
	{
		// Если над водой - отражение под водой
		reflectPos.y = waterY - abs(distanceToWater);
	}

	// Отражаем ориентацию камеры (зеркально по Y)
	...
	// Создаем новую ориентацию из отраженных векторов
	...
	reflectMat.SetColumn(2, -forward); // Инвертируем forward для правильного отражения

	Quaternion reflectOrientation;
	reflectOrientation.FromRotationMatrix(reflectMat);

	// Устанавливаем позицию и ориентацию камеры отражения
	...
	mReflectCamera->setFarClipDistance(mainCam->getFarClipDistance());
}


void MagixWorld::updateUnderWater()
{
	if (numWaterPlanes == 0) return;

	if (mSceneMgr->hasEntity("WorldWaterPlane1"))
	{
		Entity *ent = mSceneMgr->getEntity("WorldWaterPlane1");
		bool isUnderwater = mMainCamera->getIsUnderwater();

		if (isUnderwater)
		{
			// ПОД ВОДОЙ - отключаем отражения и используем подводный материал
			if (ent->getSubEntity(0)->getMaterialName() != WATER_UNDERWATERMAT)
			{
				// Отключаем отражение для камеры
				if (mReflectCamera)
				{
				...
				// Переключаем материал на подводный
				ent->setMaterialName(WATER_UNDERWATERMAT);

				// Обновляем вьюпорт отражения (только если RTT существует)
				if (mReflectRtt)
				{
		...
		{
			// НАД ВОДОЙ - включаем отражения и используем отражающий материал
			if (ent->getSubEntity(0)->getMaterialName() != WATER_REFLECTMAT)
			{
				// Включаем отражение для камеры
				...
				// Переключаем материал на отражающий
...
void MagixWorld::updateCameraReflector()
{
	if (!mReflectNode || !mRayQuery || numWaterPlanes == 0) return;

	// Бросаем луч от камеры вниз для определения ближайшей водной поверхности
	...
	mRayQuery->setQueryMask(WATER_MASK);
	...
	{
		if (i->movable && i->movable->getQueryFlags() == WATER_MASK && i->distance < 1000)
		{
			if (i->movable->getParentSceneNode())
			{
				// Обновляем позицию плоскости отражения
	...
	// Если не нашли воду под камерой, используем первую водную плоскость
	if (!waterFound && numWaterPlanes > 0)
	{
		Entity* firstWater = mSceneMgr->getEntity("WorldWaterPlane1");
		if (firstWater && firstWater->getParentSceneNode())
		{
			RefPosition = Vector3(0.0f, firstWater->getParentSceneNode()->getPosition().y, 0.0f);
			mReflectNode->setPosition(RefPosition);
		}
	}
}
...
void MagixWorld::update()
{
	...
	// ВАЖНО: Обновляем камеру отражения всегда, когда есть водные поверхности
	if (numWaterPlanes > 0)
	{
		updateReflectionCamera();

		// Принудительно обновляем RTT текстуру
...
void MagixWorld::loadWorld(const String &name)
{
	...
	if (tTreesFilename.size()>0 || tTree1 != "" || tTree2 != "" || tTree3 != "")
	{
		...
		if (mDef->pagedGeometryOn)
		{
			...
			mLargeTrees->addDetailLevel<BatchPage>(1500, (Real)(mDef->hasVertexProgram ? 300 : 0));
	...
	const String &matName, const String &playSound, bool tIsSolid)
{
	...
	// Создаем систему отражений только для первой водной поверхности
	if (numWaterPlanes == 1)
	{
		// Создаем подвижную плоскость для отражений
		mReflectPlane = new MovablePlane("ReflectPlane");
		mReflectPlane->d = 0;
		mReflectPlane->normal = Vector3::UNIT_Y;

		// Создаем RTT текстуру для отражений с большим расстоянием прорисовки
		mReflectTex = TextureManager::getSingleton().createManual(
			"RttTex",
			ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
			TEX_TYPE_2D,
			1024, 1024,  // Можно увеличить до 2048 для лучшего качества
			0,
			PF_R8G8B8,
			TU_RENDERTARGET);
		...
		// Настраиваем RTT для работы на больших расстояниях
		...
		waterPlaneEnt->setMaterialName(WATER_REFLECTMAT);
		initReflectionViewport();
	}
	else
	{
